...
const ManagementScreen: React.FC<ManagementScreenProps> = ({ setActiveScreen }) => {
    ...
    // Set specialty and role to '' to enable placeholder behavior
        ...
        try {
            switch (type) {
                case 'user': {
                    ...
                    // Add validation for specialty and role
                    if (subType === 'teacher' && !form.specialty) {
                        throw new Error('Vui lòng chọn chuyên môn cho giáo viên.');
                    }
                    if (subType === 'manager' && !form.role) {
                        throw new Error('Vui lòng chọn vai trò cho người dùng.');
                    }
                    ...
                    if (editingUser) {
                        ...
                        if (!form.password || form.password.length < 6) {
                            throw new Error('Mật khẩu là bắt buộc và phải có ít nhất 6 ký tự.');
                        }
                ...
                case 'student': {
                    if (!studentForm.courseId) {
                        throw new Error('Vui lòng chọn một khóa đào tạo cho học viên.');
                    }
        ...
        <div className="p-4 md:p-6 lg:p-8 bg-gray-100 min-h-screen">
            ...
            <div className="bg-white p-6 rounded-xl shadow-md mb-8">
                ...
                <form onSubmit={(e) => handleSubmit(e, 'user', 'teacher')} className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        <input type="text" placeholder="Tên giáo viên" value={teacherForm.name || ''} onChange={e => setTeacherForm({ ...teacherForm, name: e.target.value })} className="p-2 border rounded" required />
                        <input type="tel" placeholder="SĐT (dùng để đăng nhập)" value={teacherForm.phone || ''} onChange={e => setTeacherForm({ ...teacherForm, phone: e.target.value })} className="p-2 border rounded" required />
                        <input type="password" placeholder={editingUser ? "Mật khẩu (để trống nếu không đổi)" : "Mật khẩu (ít nhất 6 ký tự)"} value={teacherForm.password || ''} onChange={e => setTeacherForm({ ...teacherForm, password: e.target.value })} className="p-2 border rounded" autoComplete="new-password" required={!editingUser} />

                        <select
                            value={teacherForm.specialty || ""}
                            onChange={e => setTeacherForm({ ...teacherForm, specialty: e.target.value as TeacherSpecialty })}
                    ...
                    <div className="flex space-x-2"><button type="submit" disabled={isSubmitting} className="bg-blue-500 text-white p-2 rounded hover:bg-blue-600 disabled:bg-blue-300">{editingUser ? 'Cập nhật' : 'Thêm giáo viên'}</button>{editingUser && <button type="button" onClick={() => handleCancelEdit('teacher')} className="bg-gray-300 p-2 rounded hover:bg-gray-400">Hủy</button>}</div>
                </form>
                <div className="overflow-x-auto mt-6">
                    <table className="min-w-full bg-white">
                        <thead className="bg-gray-200"><tr><th className="py-2 px-4 text-left">Tên Giáo viên</th><th className="py-2 px-4 text-left">SĐT</th><th className="py-2 px-4 text-left">Chuyên môn</th><th className="py-2 px-4 text-left">Hành động</th></tr></thead>
                        <tbody>
                            {teachers.map(teacher => (<tr key={teacher.id} className="border-b"><td className="py-2 px-4">{teacher.name}</td><td className="py-2 px-4">{teacher.phone}</td><td className="py-2 px-4">{teacher.specialty}</td><td className="py-2 px-4"><button onClick={() => handleEdit('user', teacher)} className="text-blue-500 hover:underline mr-4">Sửa</button><button onClick={() => handleDeleteRequest('user', teacher.id)} className="text-red-500 hover:underline">Xóa</button></td></tr>))}
                        </tbody>
                    </table>
                </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-xl shadow-md">
                ...
                <form onSubmit={(e) => handleSubmit(e, 'user', 'manager')} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end mb-6">
                    <input type="text" placeholder="Tên người dùng" value={userForm.name} onChange={e => setUserForm({ ...userForm, name: e.target.value })} className="p-2 border rounded" required />
                    <input type="tel" placeholder="SĐT (dùng để đăng nhập)" value={userForm.phone} onChange={e => setUserForm({ ...userForm, phone: e.target.value })} className="p-2 border rounded" required />
                    <input type="password" placeholder={editingUser ? "Mật khẩu (để trống nếu không đổi)" : "Mật khẩu (ít nhất 6 ký tự)"} value={userForm.password || ''} onChange={e => setUserForm({ ...userForm, password: e.target.value })} className="p-2 border rounded" autoComplete="new-password" required={!editingUser} />
                    <select value={userForm.role || ''} onChange={e => setUserForm({ ...userForm, role: e.target.value as UserRole })} className="p-2 border rounded" required>
                    ...
                    <div className="flex space-x-2 col-span-full">
                        <button type="submit" disabled={isSubmitting} className="bg-blue-500 text-white p-2 rounded hover:bg-blue-600 disabled:bg-blue-300">{editingUser ? 'Cập nhật' : 'Thêm người dùng'}</button>
                        {editingUser && <button type="button" onClick={() => handleCancelEdit('user')} className="bg-gray-300 p-2 rounded hover:bg-gray-400">Hủy</button>}
                    </div>
                </form>
